...
def signin(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'signup.html', {'form': AuthenticationForm, 'error': 'Username or password is incorrect'})
        else:
            login(request, user)
            return redirect('tasks')
...
def task_detail(request, task_id):
    if request.method == 'GET':
        ...
        return render(request, 'task_detail.html', {'task': task, 'formulario': formulario})
    else:
        try:
            task = get_object_or_404(Task, pk=task_id, user= request.user)
            ...
            return render(request, 'task_detail.html', {'task': task, 'formulario': formulario, 'error': 'Error al alctualizar la tarea'})
